...
%% necessary geometric distances
e = (H - h - delta) / 2; % mm
d = (L - a - 2 * b) / 2; % mm

%% calculation of the average flow line (top and bottom)
% syms t
% expr_a = sqrt((a/2 * sin(t))^2 + (e/2 * cos(t))^2);
% expr_d = sqrt((d/2 * sin(t))^2 + (e/2 * cos(t))^2);
% l_ell_a = vpaintegral(expr_a, [0 pi/2]);
% l_ell_d = vpaintegral(expr_d, [0 pi/2]);
% l_e = 2 * (b + l_ell_a + l_ell_d); % mm

%% calculation of the average flow line (top only)
l_e = 2*e + b + a/2 + d;
...
while 1
    ...
    b_a = b_m_cur;
    b_d = b_m_cur * s_a * k_fe / (s_d * k_fe);
    b_e = b_m_cur * s_a * k_fe / (s_e * k_fe);

    % obtain the field strength from induction in A/m
    h_a = h_b_lin_approx(b_a) * 100; 
    h_d = h_b_lin_approx(b_d) * 100;
    h_e = h_b_lin_approx(b_e) * 100;

    h_oa = b_a / (sqrt(2) * 4 * pi * 10 ^ (-7)); % A/m
    h_od = b_d / (sqrt(2) * 4 * pi * 10 ^ (-7)); % A/m
    
    u_o = (h_oa + h_od) * delta * 10^(-3); % A
    u_a = h_a * h * 10^(-3); % A
    u_d = h_d * h * 10^(-3); % A
    u_e = 2 * h_e * l_e * 10^(-3); % A
...
r_1 = l_1 * w_1 * rho_1 * 10^(-3);
r_2 = l_2 * w_2 * rho_2 * 10^(-3);
...
v_a = a * h * c; % mm^3
v_d = d * h * c; % mm^3
v_e = e * c * (a + 2*b + 2*d); % mm^3

g_a = v_a * gamma_fe * k_fe * 10^(-9);
g_d = v_d * gamma_fe * k_fe * 10^(-9);
g_e = v_e * gamma_fe * k_fe * 10^(-9);
...
function h = h_b_lin_approx(b) 
    ...
    h_data = [0 0.3 0.6 1.6 4.8 7.2 8.6 14.2 24 40]; % A/cm
